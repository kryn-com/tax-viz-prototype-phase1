...
def test_compose_additional_ltcg_qd_income_sliver_returns_baseline_and_altered_federal_results():
    scenario = _make_scenario()

    result = compose_additional_ltcg_qd_income_sliver(
        scenario,
        additional_ltcg_qd_income=1000.0,
    )

    assert result.result_kind == "ltcg_qd_sliver"
    assert result.additional_ltcg_qd_income == 1000.0
    assert result.baseline_federal_result.scenario is scenario
    assert result.altered_federal_result.scenario is not scenario
    assert result.altered_federal_result.scenario.ltcg_qd_income == 26000.0
    assert result.altered_federal_result.scenario.ordinary_income == scenario.ordinary_income
...
def test_compose_additional_ltcg_qd_income_sliver_recomputes_full_applicable_pipeline():
    scenario = _make_scenario(ordinary_income=85000.0, social_security_income=30000.0)

    composed = compose_additional_ltcg_qd_income_sliver(
        scenario,
        additional_ltcg_qd_income=5000.0,
    )

    if hasattr(scenario, "model_copy"):
        altered_scenario = scenario.model_copy(update={"ltcg_qd_income": 30000.0})
    else:
        altered_scenario = scenario.copy(update={"ltcg_qd_income": 30000.0})
    ...
    assert composed.baseline_federal_result.ordinary_output is not None
    assert composed.altered_federal_result.ordinary_output is not None
    assert composed.baseline_federal_result.ltcg_qd_output is not None
    assert composed.altered_federal_result.ltcg_qd_output is not None


def test_compose_additional_ltcg_qd_income_sliver_preserves_separate_outputs_and_deltas():
    scenario = _make_scenario(ordinary_income=170000.0, ltcg_qd_income=70000.0)

    result = compose_additional_ltcg_qd_income_sliver(
        scenario,
        additional_ltcg_qd_income=30000.0,
    )
...
def test_compose_additional_ltcg_qd_income_sliver_unsupported_boundaries_are_explicit():
    scenario = _make_scenario(
...
def test_compose_additional_ltcg_qd_income_sliver_reflects_niit_change_from_ltcg_qd_change():
    scenario = _make_scenario(
    ...
    assert result.baseline_niit_component.niit_tax < result.altered_niit_component.niit_tax
    assert result.deltas.niit_component_delta > 0.0
    assert result.deltas.federal_niit_tax_delta == pytest.approx(
        result.deltas.niit_component_delta
    )


@pytest.mark.parametrize("increment", [0.0, -1.0])
def test_compose_additional_ltcg_qd_income_sliver_requires_positive_increment(increment):
    with pytest.raises(ValueError, match="greater than zero"):
        compose_additional_ltcg_qd_income_sliver(_make_scenario(), increment)